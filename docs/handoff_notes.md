...
**Date:** 2026-07-12
**Branch:** `feature/p6-rbac` (off `master`) — P6 work-in-progress branch, uses its own Supabase project (`cashbook.permissions`, id `khyjmbsgekldfhflcbfs`), not the hosted prod project. `master` still 1 commit ahead of `origin/master` (`89d66ef` unpushed).
...
Offline-first Android cashbook. Phases P0–P5 done, proof gates passed. P6 (RBAC backend) done on the `feature/p6-rbac` branch, proof gate passed against the `cashbook.permissions` dev project — not yet merged to master or re-pointed at the hosted prod Supabase project.
